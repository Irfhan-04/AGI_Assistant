...
class MainWindow(ctk.CTk):
    ...
    def _on_session_processed(self, workflow_learned: bool):
        ...
        Args:
            workflow_learned: Whether a workflow was learned
        """
        if workflow_learned:
    ...
    def _execute_workflow(self, workflow: dict):
        ...
        try:
            ...
            if result.get("success"):
                ...
                self.after(0, lambda: self.status_text.configure(
                    text=f"Workflow '{workflow.get('workflow_name')}' failed: {result.get('error_message')}"
                ))
            
        except Exception as e:
            logger.error(f"Error executing workflow: {e}", exc_info=True)
            self.after(0, lambda: self.status_text.configure(text="Error executing workflow"))
